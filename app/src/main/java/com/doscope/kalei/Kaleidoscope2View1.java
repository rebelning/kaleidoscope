 ...
 * r可以大于R，l可以大于r
 */
public class Kaleidoscope2View1 extends SurfaceView implements SurfaceHolder.Callback {
    private static final String LOGTAG = LogUtil.makeLogTag(Kaleidoscope2View1.class);
    ...
    public Kaleidoscope2View1(Context context) {
        super(context);
        init(context);
    }

    public Kaleidoscope2View1(Context context, AttributeSet attrs) {
        super(context, attrs);
        init(context);
    }

    public Kaleidoscope2View1(Context context, AttributeSet attrs, int defStyleAttr) {
...
//                    drawMovingORTrack(canvas);
    ...
    private void drawXY(Canvas canvas, float cx, float cy, float radius) {
        //动圆圆心所在的轨迹
        canvas.drawLine(0, cy / 2 + radius / 2 + 5, screenWidth, cy / 2 + radius / 2 + 5, drawXyPaint);
...
//        }
        ...
        for (int i = 0; i < divisor; i++) {
//            currentValue1 += 0.005;                                  // 计算当前的位置在总长度上的比例[0,1]
            currentValue1 += 2.5;                                  // 计算当前的位置在总长度上的比例[0,1]
//            if (currentValue1 >= 1) {
//                currentValue1 = 0;
//            }
