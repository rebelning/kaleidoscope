 ...
 * r可以大于R，l可以大于r
 */
public class Kaleidoscope2ViewBack extends SurfaceView implements SurfaceHolder.Callback {
    private static final String LOGTAG = LogUtil.makeLogTag(Kaleidoscope2ViewBack.class);
    ...
    public Kaleidoscope2ViewBack(Context context) {
        super(context);
        init(context);
    }

    public Kaleidoscope2ViewBack(Context context, AttributeSet attrs) {
        super(context, attrs);
        init(context);
    }

    public Kaleidoscope2ViewBack(Context context, AttributeSet attrs, int defStyleAttr) {
...
//                float radiusOr = radiusOT + radiusOR / 2;
                ...
                DebugLog.i(LOGTAG, "radiusOr-1:" + radiusOr);
                float l = radiusOR / 2;
                DebugLog.i(LOGTAG, "l:" + l);
                //动圆圆心
//                calculateTrack(cx, cy, radiusOT);
                radiusOr = radiusOr / 2 - 10;
//                l = radiusOr;
                DebugLog.i(LOGTAG, "radiusOr-2:" + radiusOr);
                calculateNewTrack(cx, cy, radiusOT, radiusOr, l);
...
//                    drawORCircle(canvas, cx, cy, radiusOR);
                    radiusOr = radiusOr / 2 - 10;
                    drawXY(canvas, cx, cy, radiusOr);
                    //Or 动圆
                    //动圆Or的轨道
//                    drawOrTrack(canvas, i, radiusOr);
                    drawOrTrack(canvas, i);

//                    drawMovingORTrack(canvas);
     ...
     * @param canvas
     */
    private void drawXY(Canvas canvas, float cx, float cy, float radius) {
        //动圆圆心所在的轨迹
        canvas.drawLine(0, cy / 2 + radius / 2 + 15, screenWidth, cy / 2 + radius / 2 + 15, drawXyPaint);
//        canvas.drawLine(0, cy / 2 + radius / 2 , screenWidth, cy / 2 + radius / 2, drawXyPaint);
//        canvas.drawLine(0, cy / 2 , screenWidth, cy / 2 , drawXyPaint);
//        canvas.drawLine(0, radius , screenWidth, radius, drawXyPaint);
        //x
...
//        canvas.drawText(xy1, x - 20, y + 20, drawTextPaint);
    ...
    private void calculateNewTrack(float x, float y, float OT, float OR, float l) {

        float x1 = screenWidth - 10;
...
//            DebugLog.i(LOGTAG, "degrees****:" + degrees);
            TrackRadiusPoint trackRadius = new TrackRadiusPoint();
//            trackRadius.cx = cx + currentValue1;
//            trackRadius.cx = deg * OR;
            trackRadius.cx = degrees * OR + 10;
//            trackRadius.cy = cy;
            trackRadius.cy = cy + OR / 2 + 15;
//            trackRadius.cy = cy + OR / 2 ;
            trackRadius.t = degrees;
            trackRadius.radius = OR;
            trackRadius.lr = l;
            trackRadiusList.add(trackRadius);
            calculateTrackPoint(x, y, OR, l, degrees);
            degrees += 0.025;
        }
